...
export const addProductToCart = async (
  productId: number,
  quantity: number,
  userId: number // pass internally, not as main param
) => {
...
export const updateCartItemQuantity = async (
  ...
  const cartItem = await prisma.cart.findFirst({
    ...
    include: { products: { select: { id: true, price: true, stock_quantity: true } } }
  });

  if (!cartItem || !cartItem.products) throw new Error('Cart item not found');

  const product = cartItem.products;

  if ((product.stock_quantity ?? 0) < newQuantity) throw new Error('Insufficient stock');

  const stockAdjustment = newQuantity - cartItem.quantity;

  await prisma.products.update({
    where: { id: product.id },
    data: { stock_quantity: { decrement: stockAdjustment }, updated_at: new Date() }
  });
  ...
  return await prisma.cart.update({
    where: { id: cartItemId },
    data: { quantity: newQuantity, price: productPrice.mul(newQuantity), updated_at: new Date() },
    include: { products: true }
  });
};
...
export const deleteCartItem = async (
  ...
  const cartItem = await prisma.cart.findFirst({
    where: { id: cartItemId, user_id: userId },
    include: { products: { select: { id: true, stock_quantity: true } } }
  });
  ...
  if (data && cartItem.products) {
    await prisma.products.update({
      where: { id: cartItem.products.id },
      data: { stock_quantity: { increment: cartItem.quantity }, updated_at: new Date() }
    });
  }
...
export const getUserCart = async (userId: number) => {
  ...
  const subtotal = cartItems.reduce((sum, item) => sum.add(new Prisma.Decimal(item.price || 0)), new Prisma.Decimal(0));
  const totalItems = cartItems.reduce((sum, item) => sum + (item.quantity || 0), 0);
  const totalDiscount = cartItems.reduce((sum, item) => {
    const discount = item.products?.discount_percentage || 0;
    const itemPrice = new Prisma.Decimal(item.price || 0);
...
// Clear user's cart
export const clearUserCart = async (userId: number) => {
  return await prisma.cart.deleteMany({ where: { user_id: userId } });
};
